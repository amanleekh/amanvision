...
// load and set avm data
#define OBJ_AVM_SET_MEM(_str1, _str2)                       \
...
/**
* load avm table.db into this struct
*/
typedef struct _avm_buffer_data_struct_
{
    float x;    /**< vert x,y,z */
    float y;
    float z;
...
/**< static data init*/
QMutex AvmGpu::m_s_mutex;
int AvmGpu::m_s_inst_id = 0;
int AvmGpu::m_s_is_share_lb = 0; 
int AvmGpu::m_s_is_share_lb_load_sts = 0;
int AvmGpu::m_s_is_share_vbo = 0;
int AvmGpu::m_s_is_share_vbo_load_sts = 0;
int AvmGpu::m_s_is_share_tex = 0;
int AvmGpu::m_s_is_share_tex_load_sts = 0;
avmgpu_share_vbo_s AvmGpu::m_s_hare_vbo;
avmgpu_share_texure_s AvmGpu::m_s_share_tex;
avmgpu_share_ltbalance_s AvmGpu::m_s_share_lb;
...
*/
AvmGpu::AvmGpu(int car_type, int proj_width, int proj_height, 
    ...
    int is_share_context_vbo, int is_share_context_texture)
{
    ...
    m_current_view = AVM_VIEW_UNKNOW;
#if GLOBAL_RUN_ENV_DESKTOP == 1
    m_is_use_bmp = 1;
#else
    ...
    if (1 == m_is_use_bmp)
    {
        AVMGPU_LOG("AvmGpu::AvmGpu. is use image file=%d\n", m_is_use_bmp);
    }
    m_car_type = car_type;
    ...
    m_dyn_trans_dst_view = AVM_VIEW_UNKNOW; 
    ...
    if (ini_get_d2_vrate(&m_2d_f_clip_rate, &m_2d_r_clip_rate) != 0)
    {
        ...
        AVMGPU_LOG("AvmGpu::AvmGpu. get clip faild, use default\n");
    }
    ...
    m_p_db_file_name = (char *)("avm_qt_app_res/data/Table.db");
    m_p_vshader_name = (char *)("avm_qt_app_res/shader/vshader_gpu.glsl");
    m_p_fshader_name = (char *)("avm_qt_app_res/shader/fshader_gpu.glsl");
    m_p_avm_mem = NULL;
    m_p_avm_idx_mem = NULL;
    m_avm_mem_st_num = 0;

    // single or two camea view, .data file name
    m_p_cam_vshader_name = (char *)("avm_qt_app_res/shader/vshader_camera.glsl");
    m_p_cam_fshader_name = (char *)("avm_qt_app_res/shader/fshader_camera.glsl");
    m_p_front_db_file_name = (char *)("avm_qt_app_res/data/table_cam_front.db");
    m_p_rear_db_file_name = (char *)("avm_qt_app_res/data/table_cam_rear.db");
    m_p_two_db_file_name = (char *)("avm_qt_app_res/data/table_cam_two.db");
    m_p_fisheye_file_name = (char *)("avm_qt_app_res/data/fisheye_clip.db");

    // opengl vbo
    m_avm_table_vbo = -1; 
    m_avm_idx_vbo = -1;
    m_avm_single_front_camera_vbo = -1;
    m_avm_single_rear_camera_vbo = -1;
    m_avm_two_camera_vbo = -1;
    m_avm_fisheye_front_vbo = -1; 
    m_avm_fisheye_rear_vbo = -1;
    m_avm_fisheye_clip_front_vbo = -1; 
    m_avm_fisheye_clip_rear_vbo = -1;
    ...
    // etc
    m_inst_id = AvmGpu::m_s_inst_id++;

    // static val init
    if (0 == m_inst_id)
    {
        AvmGpu::m_s_is_share_vbo = is_share_context_vbo;
        AvmGpu::m_s_is_share_tex = is_share_context_texture;
        AvmGpu::m_s_is_share_lb = 1;   // default light balance is share
        AvmGpu::m_s_is_share_vbo_load_sts = 0;
        AvmGpu::m_s_is_share_tex_load_sts = 0;
        AvmGpu::m_s_is_share_lb_load_sts = 0;
        // lb
        for (i = 0; i < 4; i++)
        {
            AvmGpu::m_s_share_lb.m_color_k[i] = 1.0f;
        }
        AvmGpu::m_s_share_lb.m_fps = DEFAULT_FPS;
        // vbo
        AvmGpu::m_s_hare_vbo.m_avm_table_vbo = -1;
        AvmGpu::m_s_hare_vbo.m_avm_idx_vbo = -1;
        AvmGpu::m_s_hare_vbo.m_avm_single_front_camera_vbo = -1;
        AvmGpu::m_s_hare_vbo.m_avm_single_rear_camera_vbo = -1;
        AvmGpu::m_s_hare_vbo.m_avm_two_camera_vbo = -1;
        AvmGpu::m_s_hare_vbo.m_avm_fisheye_front_vbo = -1;
        AvmGpu::m_s_hare_vbo.m_avm_fisheye_rear_vbo = -1;
        AvmGpu::m_s_hare_vbo.m_avm_fisheye_clip_front_vbo = -1;
        AvmGpu::m_s_hare_vbo.m_avm_fisheye_clip_rear_vbo = -1;
        AvmGpu::m_s_hare_vbo.m_avm_mem_st_num = 0;
        // tex
        for (i = 0; i < 4; i++)
        {
            AvmGpu::m_s_share_tex.m_texture_id[i] = -1;
            AvmGpu::m_s_share_tex.m_texture_vaddr[i] = NULL;
            AvmGpu::m_s_share_tex.m_texture_paddr[i] = 0;
        }
        AvmGpu::m_s_share_tex.m_tex_uid_h = 0;
        AvmGpu::m_s_share_tex.m_tex_uid_l = 0;
        AvmGpu::m_s_share_tex.m_tex_updated = 0;
        for (i = 0; i < 4; i++)
        {
            AvmGpu::m_s_share_tex.m_video_fc[i] = 1;
        }
    }

    AVM_LOG("AvmGpu::AvmGpu. share_vbo=%d, share_tex=%d, share_lb=%d, inst_id=%d\n",
        AvmGpu::m_s_is_share_vbo, AvmGpu::m_s_is_share_tex, AvmGpu::m_s_is_share_lb,
        m_inst_id);
}
...
*/
int AvmGpu::avmgpu_init()
{
    int i;
    int ret;
    
    // env init, compile shader and create opengl program
    ret = avm_gpu_env_init();
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avmgpu_init. avm_gpu_env_init faild\n");
        return 1;
    }

    // gen vbo
    ret = avm_gpu_vbo_init();
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avmgpu_init. avm_gpu_vbo_init faild\n");
        return 1;
    }

    // gen texture
    ret = avm_gpu_texture_init();
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avmgpu_init. avm_gpu_texture_init faild\n");
        return 1;
    }
    ...
    ret = avm_gpu_load_db_fisheye(m_p_fisheye_file_name);
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avmgpu_init. load fisheye faild\n");
        return 1;
    }
	...
	if (1 == m_is_use_bmp)
	{
        m_img1.load("avm_qt_app_res/data/cap_right.jpg");
        m_img2.load("avm_qt_app_res/data/cap_front.jpg");
        m_img3.load("avm_qt_app_res/data/cap_left.jpg");
        m_img4.load("avm_qt_app_res/data/cap_rear.jpg");
    ...
    // set init eye pos
    ret = avm_gpu_postion_init();
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avmgpu_init. avm_gpu_postion_init faild\n");
        return 1;
    }

    // light balance init
    ret = avm_gpu_light_balance_init();
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avmgpu_init. avm_gpu_light_balance_init faild\n");
        return 1;
    }

    AVMGPU_LOG("AvmGpu::avmgpu_init success, inst_id=%d\n", m_inst_id);
    return 0;
}



int AvmGpu::avm_gpu_init_avm_table_data()
{
    int ret;

    unsigned int start_time = get_time_ms();

    // load avm stitch table data
    ret = load_db_table(m_p_db_file_name, &m_p_avm_mem, 
        &m_p_avm_idx_mem, &m_avm_mem_st_num);
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_init_avm_table_data. load stitch data for stitch faild\n");
        return 1;
    }

    // load single camera data(with fish fisheye correction)
#if SUPPORT_FRONT_REAR_CALIB == 1
    ret = avm_gpu_load_db_camera(m_p_front_db_file_name, &m_p_front_cam_mem, &m_front_cam_point_num);
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_init_avm_table_data. load cam front(fisheye correction) faild\n");
        return 1;
    }
    ret = avm_gpu_load_db_camera(m_p_rear_db_file_name, &m_p_rear_cam_mem, &m_rear_cam_point_num);
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_init_avm_table_data. load cam rear(fisheye correction) faild\n");
        return 1;
    }
#endif

    // load two cam data(left and right, with fish fisheye correction)
    ret = avm_gpu_load_db_camera(m_p_two_db_file_name, &m_p_two_cam_mem, &m_two_cam_point_num);
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_init_avm_table_data. load cam two(fisheye correction) faild\n");
        return 1;
    }
    ...
    AVM_LOG("AvmGpu::avm_gpu_init_avm_table_data. inst_id=%d, cost %d ms, struct num=%d\n",
        m_inst_id, det_time, m_avm_mem_st_num);

    return 0;
}


unsigned int AvmGpu::get_time_ms()
{
    struct timeval  tv;
...
/**< u64++ */
void AvmGpu::uint64_inc(unsigned int *p_h, unsigned int *p_l)
{
    (*p_l)++;
    if (0x00 == *p_l)
    {
...
/**< u64 comp */
int AvmGpu::uint64_cmp(unsigned int *p1_h, unsigned int *p1_l, unsigned int *p2_h, unsigned int *p2_l)
{
    if (*p1_h > *p2_h)
    {
        return 1;
...
/**< u64 set */
void AvmGpu::uint64_set(unsigned int *p1_h, unsigned int *p1_l, unsigned int *p2_h, unsigned int *p2_l)
{
    *p1_h = *p2_h;
    *p1_l = *p2_l;
}
...
/**< lock */
void AvmGpu::share_lock()
{
    //qDebug("--lock %s\n", __FUNCTION__);
    AvmGpu::m_s_mutex.lock();
}


/**< unlock */
void AvmGpu::share_unlock()
{
    //qDebug("--unlock %s\n", __FUNCTION__);
    AvmGpu::m_s_mutex.unlock();
}


/**
 * @brief AvmGpu::avm_gpu_env_init
 * @return
 */
int AvmGpu::avm_gpu_env_init()
{
    int ret;
...
#define AVM_GPU_FUNC_SET(_val, type, _str)    \
    ...
    AVM_GPU_FUNC_SET(g_pFNglTexDirectVIV, PFNGLTEXDIRECTVIV, "glTexDirectVIV")
    AVM_GPU_FUNC_SET(g_pFNglTexDirectVIVMap, PFNGLTEXDIRECTVIVMAP, "glTexDirectVIVMap")
    AVM_GPU_FUNC_SET(g_pFNglTexDirectInvalidateVIV, PFNGLTEXDIRECTINVALIDATEVIV, "glTexDirectInvalidateVIV")
#undef AVM_GPU_FUNC_SET
#endif
    ...
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_env_init. shader_load_str faild.\n");
        return 1;
    }
    ...
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_env_init. shader_load_str camera faild.\n");
        return 1;
    }
...
int AvmGpu::avm_gpu_load_data_vbo()
{
    // avm stitch data vert vbo
    if (NULL == m_p_avm_mem)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_load_data_vbo. m_p_avm_mem is null\n");
        return 1;
    }
    glGenBuffers(1, &m_avm_table_vbo);
    glBindBuffer(GL_ARRAY_BUFFER, m_avm_table_vbo);
    glBufferData(GL_ARRAY_BUFFER, m_avm_mem_st_num * sizeof(avm_buffer_data_s), m_p_avm_mem, GL_STATIC_DRAW);

    // avm index buffer
    if (NULL == m_p_avm_idx_mem)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_load_data_vbo. m_p_avm_idx_mem is null\n");
        return 1;
    }
    glGenBuffers(1, &m_avm_idx_vbo);
    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, m_avm_idx_vbo);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, m_avm_mem_st_num / 4 * 6 * sizeof(unsigned int), m_p_avm_idx_mem, GL_STATIC_DRAW);

    // single front camera
#if SUPPORT_FRONT_REAR_CALIB == 1
    if (NULL == m_p_front_cam_mem)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_load_data_vbo. m_p_front_cam_mem(for fisheye correction) is null\n");
        return 1;
    }
    glGenBuffers(1, &m_avm_single_front_camera_vbo);
    glBindBuffer(GL_ARRAY_BUFFER, m_avm_single_front_camera_vbo);
    ...
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_load_data_vbo. m_p_rear_cam_mem(for fisheye correction) is null\n");
        return 1;
    }
    glGenBuffers(1, &m_avm_single_rear_camera_vbo);
    glBindBuffer(GL_ARRAY_BUFFER, m_avm_single_rear_camera_vbo);
    ...
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_load_data_vbo. m_p_two_cam_mem is null\n");
        return 1;
    }
    glGenBuffers(1, &m_avm_two_camera_vbo);
    glBindBuffer(GL_ARRAY_BUFFER, m_avm_two_camera_vbo);
    ...
    glGenBuffers(1, &m_avm_fisheye_front_vbo);
    glBindBuffer(GL_ARRAY_BUFFER, m_avm_fisheye_front_vbo);
    ...
    glGenBuffers(1, &m_avm_fisheye_rear_vbo);
    glBindBuffer(GL_ARRAY_BUFFER, m_avm_fisheye_rear_vbo);
    ...
    glGenBuffers(1, &m_avm_fisheye_clip_front_vbo);
    glBindBuffer(GL_ARRAY_BUFFER, m_avm_fisheye_clip_front_vbo);
    ...
    glGenBuffers(1, &m_avm_fisheye_clip_rear_vbo);
    glBindBuffer(GL_ARRAY_BUFFER, m_avm_fisheye_clip_rear_vbo);
...
int AvmGpu::avm_gpu_vbo_init()
{
    int ret;
    
    // check vbo share status
    share_lock();
    if (AvmGpu::m_s_is_share_vbo)
    {
        // have not load vbo
        if (0 == AvmGpu::m_s_is_share_vbo_load_sts)
        {
            // load .data file to mem
            ret = avm_gpu_init_avm_table_data();
            if (ret != 0)
            {
                AvmGpu::m_s_is_share_vbo_load_sts = -1;
                share_unlock();
                AVMGPU_ERR("AvmGpu::avmgpu_init. avm_gpu_init_avm_table_data faild\n");
                return 2;
            }   
            // create vert VBO buffer
            ret = avm_gpu_load_data_vbo();
            if (ret != 0)
            {
                AvmGpu::m_s_is_share_vbo_load_sts = -1;
                share_unlock();
                AVMGPU_ERR("AvmGpu::avmgpu_init. avm_gpu_load_data_vbo faild\n");
                return 3;
            }
            AvmGpu::m_s_hare_vbo.m_avm_table_vbo = m_avm_table_vbo;
            AvmGpu::m_s_hare_vbo.m_avm_idx_vbo = m_avm_idx_vbo;
            AvmGpu::m_s_hare_vbo.m_avm_single_front_camera_vbo = m_avm_single_front_camera_vbo;
            AvmGpu::m_s_hare_vbo.m_avm_single_rear_camera_vbo = m_avm_single_rear_camera_vbo;
            AvmGpu::m_s_hare_vbo.m_avm_two_camera_vbo = m_avm_two_camera_vbo;
            AvmGpu::m_s_hare_vbo.m_avm_fisheye_front_vbo = m_avm_fisheye_front_vbo;
            AvmGpu::m_s_hare_vbo.m_avm_fisheye_rear_vbo = m_avm_fisheye_rear_vbo;
            AvmGpu::m_s_hare_vbo.m_avm_fisheye_clip_front_vbo = m_avm_fisheye_clip_front_vbo;
            AvmGpu::m_s_hare_vbo.m_avm_fisheye_clip_rear_vbo = m_avm_fisheye_clip_rear_vbo;          
            AvmGpu::m_s_hare_vbo.m_avm_mem_st_num = m_avm_mem_st_num;
            AvmGpu::m_s_hare_vbo.m_front_cam_point_num = m_front_cam_point_num;
            AvmGpu::m_s_hare_vbo.m_rear_cam_point_num = m_rear_cam_point_num;
            AvmGpu::m_s_hare_vbo.m_two_cam_point_num = m_two_cam_point_num;
            AvmGpu::m_s_is_share_vbo_load_sts = 1;
            share_unlock();
            AVMGPU_LOG("AvmGpu::avm_gpu_vbo_init. inst_id=%d, act gen vbo success\n", m_inst_id);
        }
        // already load vbo success
        else if (1 == AvmGpu::m_s_is_share_vbo_load_sts)
        {
            m_avm_table_vbo = AvmGpu::m_s_hare_vbo.m_avm_table_vbo;
            m_avm_idx_vbo = AvmGpu::m_s_hare_vbo.m_avm_idx_vbo;
            m_avm_single_front_camera_vbo = AvmGpu::m_s_hare_vbo.m_avm_single_front_camera_vbo;
            m_avm_single_rear_camera_vbo = AvmGpu::m_s_hare_vbo.m_avm_single_rear_camera_vbo;
            m_avm_two_camera_vbo = AvmGpu::m_s_hare_vbo.m_avm_two_camera_vbo;
            m_avm_fisheye_front_vbo = AvmGpu::m_s_hare_vbo.m_avm_fisheye_front_vbo;
            m_avm_fisheye_rear_vbo = AvmGpu::m_s_hare_vbo.m_avm_fisheye_rear_vbo;
            m_avm_fisheye_clip_front_vbo = AvmGpu::m_s_hare_vbo.m_avm_fisheye_clip_front_vbo;
            m_avm_fisheye_clip_rear_vbo = AvmGpu::m_s_hare_vbo.m_avm_fisheye_clip_rear_vbo;
            m_avm_mem_st_num = AvmGpu::m_s_hare_vbo.m_avm_mem_st_num;
            m_front_cam_point_num = AvmGpu::m_s_hare_vbo.m_front_cam_point_num;
            m_rear_cam_point_num = AvmGpu::m_s_hare_vbo.m_rear_cam_point_num;
            m_two_cam_point_num = AvmGpu::m_s_hare_vbo.m_two_cam_point_num;
            share_unlock();
            AVMGPU_LOG("AvmGpu::avm_gpu_vbo_init. inst_id=%d, share vbo success\n", m_inst_id);
        }
        // prev load vbo faild, return error
        else
        {
            share_unlock();
            AVMGPU_ERR("AvmGpu::avmgpu_init. inst_id=%d, avm_gpu_init_avm_table_data faild, since prev load faild\n", m_inst_id);
            return 3;
        }
    }
    else
...
/**< @brief avm_gpu_texture_init, create textureb*/
int AvmGpu::avm_gpu_texture_init()
{
    int i, j;
    int texture_num_gen = 4;    // camera number

    share_lock();
    if (AvmGpu::m_s_is_share_tex)
    {
        // have not generate tex
        if (0 == AvmGpu::m_s_is_share_tex_load_sts)
        {
            ...
            for (i = 0; i < texture_num_gen; i++)
            {
                AvmGpu::m_s_share_tex.m_texture_id[i] = m_texture_id[i];
            }
            AvmGpu::m_s_is_share_tex_load_sts = 1;
            share_unlock();
            AVMGPU_LOG("AvmGpu::avm_gpu_texture_init. inst_id=%d, act gen tex success\n", m_inst_id);
        }
        // have generated
        else if (1 == AvmGpu::m_s_is_share_tex_load_sts)
        {
            for (i = 0; i < texture_num_gen; i++)
            {
                m_texture_id[i] = AvmGpu::m_s_share_tex.m_texture_id[i];
            }
            share_unlock();
            AVMGPU_LOG("AvmGpu::avm_gpu_texture_init. inst_id=%d, share tex success\n", m_inst_id);
        }
        // prev gentex faild
        else
        {
            share_unlock();
            AVMGPU_ERR("AvmGpu::avm_gpu_texture_init faild. prev gen faild\n");
            return 1;
        }
    }
    else
    ...
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_texture_init. read config for uniform2tex file faild, use default\n");
    ...
    for (i = 0; i < texture_num_gen; i++)
    {
        ...
        AVMGPU_LOG("AvmGpu::avm_gpu_texture_init. inst_id=%d, tex chn[%d] - uniform[%d]\n",
            m_inst_id, i, j);
    }
...
/**
 * @brief AvmGpu::avm_gpu_postion_init
 * @param obj_width
 * @param obj_height
 * @return
 */
int AvmGpu::avm_gpu_postion_init()
{
    ...
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_postion_init. read config file faild\n");
        return 1;
    }
...
int AvmGpu::avm_gpu_light_balance_init()
{
    int ret;
    
    share_lock();
    if (AvmGpu::m_s_is_share_lb)
    {
        // prev not load
        if (0 == AvmGpu::m_s_is_share_lb_load_sts)
        {
        	// load light data, also from stitch table data file
            ret = avm_gpu_load_db_light(m_p_db_file_name);
            if (ret != 0)
            {
                AvmGpu::m_s_is_share_lb_load_sts = -1;
                share_unlock();
                AVMGPU_ERR("AvmGpu::avm_gpu_light_balance_init. load stitch data for light balance faild\n");
                return 1;
            }
            ...
            AvmGpu::m_s_is_share_lb_load_sts = 1;
            share_unlock();
            AVMGPU_LOG("AvmGpu::avm_gpu_light_balance_init. inst_id=%d, load lb success\n", m_inst_id);
        }
        // prev load faild
        else if (-1 == AvmGpu::m_s_is_share_lb_load_sts)
        {
            share_unlock();
            AVMGPU_ERR("AvmGpu::avm_gpu_light_balance_init. prev load data faild\n");
            return 1;            
        }
        // prev load success
        else
        {
            share_unlock();
            AVMGPU_LOG("AvmGpu::avm_gpu_light_balance_init. inst_id=%d, will share lb\n", m_inst_id);
        }
    }
    else
    {
...
int AvmGpu::avm_gpu_set_view(int view_enum, int is_force)
{
    ...
    if (AVM_VIEW_CAMERA_FRONT == view_enum)
    {
    ...
    else if (AVM_VIEW_CAMERA_REAR == view_enum)
    {
    ...
    else if ((AVM_VIEW_CAMERA_LEFTRIGHT == view_enum)
        || (AVM_VIEW_CAMERA_LEFTRIGHT_CPY == view_enum))
    {
    ...
    // fisheye view
    else if (AVM_VIEW_CAMERA_FISH_FRONT == view_enum)
    {
        m_gl_model = glm::mat4(1.0f);
        m_gl_view = glm::mat4(1.0f);
    ...
    else if (AVM_VIEW_CAMERA_FISH_REAR == view_enum)
    {  
        m_gl_model = glm::mat4(1.0f);
        m_gl_view = glm::mat4(1.0f);
    ...
    else if (AVM_VIEW_CAMERA_FISH_CLIP_FRONT == view_enum)
    { 
        m_gl_model = glm::mat4(1.0f);
        m_gl_view = glm::mat4(1.0f);
    ...
    else if (AVM_VIEW_CAMERA_FISH_CLIP_REAR == view_enum)
    {
        m_gl_model = glm::mat4(1.0f);
        m_gl_view = glm::mat4(1.0f);
    ...
    else
    {
        ...
        if ((AVM_VIEW_AUTO == view_enum)
            || (AVM_VIEW_3D_FRONT == view_enum)
            || (AVM_VIEW_3D_REAR == view_enum)
            || (AVM_VIEW_3D_LEFT == view_enum)
            || (AVM_VIEW_3D_RIGHT == view_enum)
            || (AVM_VIEW_3D_LEFT_FRONT == view_enum)
            || (AVM_VIEW_3D_LEFT_CENTER == view_enum)
            || (AVM_VIEW_3D_RIGHT_FRONT == view_enum)
            || (AVM_VIEW_3D_RIGHT_CENTER == view_enum))
        {
            float view_deg = avm_gpu_get_view_deg(view_enum);
            avm_gpu_set_rotate(1, 0, view_deg);            
        }
        else if (AVM_VIEW_2D_FRONT == view_enum)
        {
        ...
        else if (AVM_VIEW_2D_REAR == view_enum)
        {
            m_gl_view = glm::lookAt(
...
float AvmGpu::avm_gpu_get_view_deg(int view_enum)
{
    if (AVM_VIEW_3D_FRONT == view_enum)
    {
        return 90.0f;
    }
    else if (AVM_VIEW_3D_LEFT == view_enum)
    {
        return -45.0f;
    }
    else if (AVM_VIEW_3D_RIGHT == view_enum)
    {
        return -135.0f;
    }
    else if (AVM_VIEW_3D_REAR == view_enum)
    {
        return -90.0f;
    }
    else if (AVM_VIEW_3D_LEFT_FRONT == view_enum)
    {
        return 45.0f;
    }
    else if (AVM_VIEW_3D_LEFT_CENTER == view_enum)
    {
        return 0.0f;
    }
    else if (AVM_VIEW_3D_RIGHT_FRONT == view_enum)
    {
        return 135.0f;
    }
    else if (AVM_VIEW_3D_RIGHT_CENTER == view_enum)
    {
        return -180.0f;
    }
    // top-down view's deg is same as rear view
    else if (AVM_VIEW_AUTO == view_enum)
    {
        return -90.0f;
    }
...
int AvmGpu::avm_gpu_check_in_3d_view(int view_enum)
{
    if ((view_enum != AVM_VIEW_3D_FRONT)
        && (view_enum != AVM_VIEW_3D_LEFT)
        && (view_enum != AVM_VIEW_3D_RIGHT)
        && (view_enum != AVM_VIEW_3D_REAR)
        && (view_enum != AVM_VIEW_3D_LEFT_FRONT)
        && (view_enum != AVM_VIEW_3D_LEFT_CENTER)
        && (view_enum != AVM_VIEW_3D_RIGHT_FRONT)
        && (view_enum != AVM_VIEW_3D_RIGHT_CENTER)
        && (view_enum != AVM_VIEW_2D_FRONT)
        && (view_enum != AVM_VIEW_2D_REAR)
        && (view_enum != AVM_VIEW_3D_FREE))
    {
        return 0;
    }
    else
...
int AvmGpu::avm_gpu_check_in_pure_3d_view(int view_enum)
{
    if ((view_enum != AVM_VIEW_3D_FRONT)
        && (view_enum != AVM_VIEW_3D_LEFT)
        && (view_enum != AVM_VIEW_3D_RIGHT)
...
void AvmGpu::avm_gpu_hide_stitch(int is_hide_right, int is_hide_front,
...
int AvmGpu::avm_gpu_set_view_dyn_trans(int view_enum)
{
    if ((m_current_view == view_enum) && (0 == m_is_in_view_trans))
    {
        return 1;
    }
    if (avm_gpu_check_in_pure_3d_view(view_enum) != 1)
    {
        return 1;
    }
    m_dyn_trans_dst_view = view_enum;
    float target_deg = avm_gpu_get_view_deg(view_enum); /* [csj 20180929-8] */
...
int AvmGpu::avm_gpu_view_dyn_process()
{
    ...
    if (avm_gpu_check_in_pure_3d_view(m_current_view) != 1)
    {
        m_is_in_view_trans = 0;
        return 0;
    }
    ...
    {
        float view_deg = avm_gpu_get_view_deg(m_dyn_trans_dst_view);
        avm_gpu_set_rotate(1, 0, view_deg);
    ...
    else
    {
        ...
        avm_gpu_set_rotate_det_angle(single_angle);
...
int AvmGpu::avm_gpu_set_rotate(int is_angle, int x_det, float view_angle)
{
    ...
    if ((AVM_VIEW_CAMERA_FRONT == m_current_view)
        || (AVM_VIEW_CAMERA_REAR == m_current_view)
        || (AVM_VIEW_CAMERA_LEFTRIGHT == m_current_view)
        || (AVM_VIEW_CAMERA_LEFTRIGHT_CPY == m_current_view)
        || (AVM_VIEW_2D_FRONT == m_current_view)
        || (AVM_VIEW_2D_REAR == m_current_view)
        || (AVM_VIEW_CAMERA_FISH_FRONT == m_current_view)
        || (AVM_VIEW_CAMERA_FISH_REAR == m_current_view)
        || (AVM_VIEW_CAMERA_FISH_CLIP_FRONT == m_current_view)
        || (AVM_VIEW_CAMERA_FISH_CLIP_REAR == m_current_view))        
    {
        return 0;
    }
    ...
    // 3d view rotate
    if (1 == avm_gpu_check_in_pure_3d_view(m_current_view))
    {
    ...
    // auto view's rotate
    else if (AVM_VIEW_AUTO == m_current_view)
    {
...
* @param x_det
*/
int AvmGpu::avm_gpu_omni_directional_rotate(int x_det, int y_det)
{
	// single and two camera can't rotate
    if ((AVM_VIEW_CAMERA_FRONT == m_current_view)
        || (AVM_VIEW_CAMERA_REAR == m_current_view)
...
#define Z_DEG_LIMIT_MIN     (2.0)
    ...
    // change view to AVM_3D_FREE
    m_current_view = AVM_VIEW_3D_FREE;
...
int AvmGpu::avm_gpu_set_rotate_det_angle(float det_angle)
{
    // float angle = x_det * 1.0 / m_projWidth * 180;  // proj_width as 180 deg
    int det = (int)(det_angle / 180.0 * m_projWidth);
    return avm_gpu_set_rotate(0, det, 0);
}


void AvmGpu::avm_gpu_get_v_p(glm::mat4 *p_v, glm::mat4 *p_p)
{
    if (p_v != NULL)
    {
        *p_v = m_gl_view;
...
/**< update viv texture */
int AvmGpu::avm_gpu_update_viv_gltex()
{
#if GLOBAL_RUN_ENV_DESKTOP == 0  
    int i;
...
/**< update image tex */
int AvmGpu::avm_gpu_update_bmp_gltex()
{
    int i;
...
*/
int AvmGpu::avm_gpu_update_dev_img_addr()
{
#if GLOBAL_RUN_ENV_DESKTOP == 0  
    int i;
    int ret;
...
/**< update image tex */
int AvmGpu::avm_gpu_update_pcenv_img_addr(int is_bmp_seq)
{
    ...
    for (i = 0; i < 4; i++)
    {
        QImage tmp_img;
        memset(str[i], 0, 128);
        sprintf(str[i], str_path[i], AvmGpu::m_s_share_tex.m_video_fc[i]);
        ...
        {
            AVMGPU_ERR("AvmGpu::avm_gpu_update_pcenv_img_addr. tex res not match\n");
            continue;
        }
        (*p_qimage[i]) = tmp_img;
        (*p_qimage[i]) = (*p_qimage[i]).convertToFormat(QImage::Format_RGBA8888);
        AvmGpu::m_s_share_tex.m_video_fc[i]++;
    }
    for (i = 0; i < 4; i++)
    {
        m_texture_vaddr[i] = (*p_qimage[i]).bits();
...
int AvmGpu::avm_gpu_update_img_addr()
{
    int i;
    ...
    if (AvmGpu::m_s_is_share_tex)
    {
        ...
        if (uint64_cmp(&m_tex_uid_h, &m_tex_uid_l, 
            &AvmGpu::m_s_share_tex.m_tex_uid_h, 
            &AvmGpu::m_s_share_tex.m_tex_uid_l) > 0)
        {
            if (0 == m_is_use_bmp)
            {
                // only imx6 env can use cam image        
                int ret = avm_gpu_update_dev_img_addr();
                if (ret != 0)
                {
                    share_unlock();
                    AVMGPU_ERR("AvmGpu::avm_gpu_update_img_addr. update image faild\n");
                }
            }
            else
            {
                avm_gpu_update_pcenv_img_addr(0);
            }
            for (i = 0; i < 4; i++)
            {
                AvmGpu::m_s_share_tex.m_texture_vaddr[i] = m_texture_vaddr[i];
                AvmGpu::m_s_share_tex.m_texture_paddr[i] = m_texture_paddr[i];
            }
            // new image address is set, but opengl texture is not updated
            AvmGpu::m_s_share_tex.m_tex_updated = 0;
            uint64_set(&AvmGpu::m_s_share_tex.m_tex_uid_h, 
                &AvmGpu::m_s_share_tex.m_tex_uid_l, 
        ...
        {       
            for (i = 0; i < 4; i++)
            {
                m_texture_vaddr[i] = AvmGpu::m_s_share_tex.m_texture_vaddr[i];
                m_texture_paddr[i] = AvmGpu::m_s_share_tex.m_texture_paddr[i];
            }
    ...
    for (i = 0; i < 4; i++)
    {
        ...
        {
            AVMGPU_LOG("AvmGpu::avm_gpu_update_img_addr. image address is null\n");
            return 1;
        }
    }
...
int AvmGpu::avm_gpu_process_pre_texture()
{
    int i;
    ...
    if (AvmGpu::m_s_is_share_tex)
    {
        ...
        if (0 == AvmGpu::m_s_share_tex.m_tex_updated)
        {
            if (0 == m_is_use_bmp)
            {
                avm_gpu_update_viv_gltex();
            }
            else
            {
                avm_gpu_update_bmp_gltex();
            }
            AvmGpu::m_s_share_tex.m_tex_updated = 1;
        }
...
*/
int AvmGpu::avm_gpu_process()
{
    // update texture image source address
    if (avm_gpu_update_img_addr() != 0)
    {
        return 1;
    }

    // in 3d view, when change view state, dyn change rotate angle
    avm_gpu_view_dyn_process();

    // light balance process, calc light param K
    avm_gpu_light_balance_process();

    if ((1 == avm_gpu_check_in_3d_view(m_current_view)) 
        || (AVM_VIEW_AUTO == m_current_view))
    {
        avm_gpu_stitch_process();
    }
    else if (AVM_VIEW_CAMERA_FRONT == m_current_view)
    {
#if SUPPORT_FRONT_REAR_CALIB == 0
        AVMGPU_ERR("AvmGpu::avm_gpu_process. AVM_VIEW_CAMERA_FRONT is not support\n");
        return 1;
#else
        avm_gpu_normal_front_process(0);
#endif      
    }
    else if (AVM_VIEW_CAMERA_REAR == m_current_view)
    {
#if SUPPORT_FRONT_REAR_CALIB == 0    
        AVMGPU_ERR("AvmGpu::avm_gpu_process. AVM_VIEW_CAMERA_FRONT is not support\n");
        return 1;
#else
        avm_gpu_normal_rear_process(0);
#endif
    }
    else if ((AVM_VIEW_CAMERA_LEFTRIGHT == m_current_view)
        || (AVM_VIEW_CAMERA_LEFTRIGHT_CPY == m_current_view))
    {
        avm_gpu_normal_two_process();
    }
    else if (AVM_VIEW_CAMERA_FISH_FRONT == m_current_view)
    {
        avm_gpu_normal_front_process(1);
    }
    else if (AVM_VIEW_CAMERA_FISH_REAR == m_current_view)
    {
        avm_gpu_normal_rear_process(1);
    }
    else if (AVM_VIEW_CAMERA_FISH_CLIP_FRONT == m_current_view)
    {
        avm_gpu_normal_front_process(2);
    }
    else if (AVM_VIEW_CAMERA_FISH_CLIP_REAR == m_current_view)
    {
        avm_gpu_normal_rear_process(2);
    }
    else
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_process. view is not vaild\n");
        return 1;
    }	
...
int AvmGpu::avm_gpu_stitch_process()
{
    glUseProgram(shader_get_program(m_shader_handler));

    // load texture
    int ret = avm_gpu_process_pre_texture();
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_process. avm_gpu_process_pre_texture faild\n");
        return 1;
    }
    ...
    glDrawElements(GL_TRIANGLES, m_avm_mem_st_num / 4 * 6 , GL_UNSIGNED_INT, (void *)0);
...
*/
int AvmGpu::avm_gpu_normal_front_process(int type)
{
    if (type < 0 || type > 2)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_normal_front_process. invild param\n");
        return 1;
    }
    ...
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_normal_front_process. avm_gpu_process_pre_texture faild\n");
        return 1;
    }
...
*/
int AvmGpu::avm_gpu_normal_rear_process(int type)
{
    if (type < 0 || type > 2)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_normal_rear_process. invild param\n");
        return 1;
    }
    ...
    if (ret != 0)
    {
        AVMGPU_ERR("AvmGpu::avm_gpu_normal_rear_process. avm_gpu_process_pre_texture faild\n");
        return 1;
    }
...
int AvmGpu::avm_gpu_normal_two_process()
{
    glUseProgram(shader_get_program(m_cam_shader_handler));
...
/**< 
* @brief load db data to struct(avm_buffer_data_s) array 
        ...
        unsigned int **p_idx_arr, int *p_st_arr_num)
{
    ...
    *p_st_arr_num = 0;
    ...
    {
        AVM_ERR("AvmGpu::load_db_table. stat file faild\n");
        return 1;
    }
    unsigned long filesize = statbuff.st_size;
    int quad_num = filesize / sizeof(quad);
    float *p_mem = (float *)memalign(128, quad_num * 4 * sizeof(avm_buffer_data_s));
    if (NULL == p_mem)
    {
        AVM_ERR("AvmGpu::load_db_table. alloc avm mem faild\n");
        return 1;
    }
    unsigned int *p_idx_mem = (unsigned int *)memalign(128, quad_num * 6 * sizeof(unsigned int));
    if (NULL == p_idx_mem)
    {
        AVM_ERR("AvmGpu::load_db_table. alloc avm idx mem faild\n");
        return 1;
    }
    *p_buffer_arr = p_mem;
    *p_idx_arr = p_idx_mem;
    ...
    if (file_r == NULL)
    {
        AVM_ERR("AvmGpu::load_db_table. fopen failed\n");
        return 1;
    }
    int i;
    quad st;
...
#define OBJ_AVM_SET_MEM(_str1, _str2)                       \
        ...
        OBJ_AVM_SET_MEM(LeftUp, _LeftUp);
        OBJ_AVM_SET_MEM(LeftDown, _LeftDown);
        OBJ_AVM_SET_MEM(RightDown, _RightDown);
        OBJ_AVM_SET_MEM(RightUp, _RightUp);
...
#undef OBJ_AVM_SET_MEM
    }
    fclose(file_r);

    *p_st_arr_num = vaild_quad_num * 4;

    AVMGPU_LOG("AvmGpu::load_db_table. inst_id=%d, file='%s', data mem have 'avm_buffer_data_s' num=%d\n", 
...
int AvmGpu::avm_gpu_load_db_light(char *p_file)
{
    struct stat statbuff;
    if (stat(p_file, &statbuff) < 0)
    {
        AVM_ERR("AvmGpu::avm_gpu_load_db_light. stat file faild\n");
        return 1;
    }
    unsigned long filesize = statbuff.st_size;
    int quad_num = filesize / sizeof(quad);
    ...
    if (file_r == NULL)
    {
        AVM_ERR("AvmGpu::avm_gpu_load_db_light. fopen failed\n");
        return 1;
    }
    int i;
    quad st;
    ...
    AVM_LOG("AvmGpu::avm_gpu_load_db_light. [1]=%d, [2]=%d, [3]=%d, [4]=%d\n",
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        {
            AVM_ERR("AvmGpu::avm_gpu_load_db_light. alloc %d light area mem faild\n", i);
            return 1;
        }
    ...
    AVM_LOG("AvmGpu::avm_gpu_load_db_light. act_num [1]=%d, [2]=%d, [3]=%d, [4]=%d\n",
...
void AvmGpu::avm_gpu_light_calc_avg_rgb(int area_id, unsigned char *p_img)
{
    int i;
    int point_cnt = AvmGpu::m_light_area_num[area_id] / 2;
#if GLOBAL_RUN_ENV_DESKTOP == 1
    ...
    {
        int x = AvmGpu::m_p_light_area[area_id][2*i];
        int y = AvmGpu::m_p_light_area[area_id][2*i+1];
...
int AvmGpu::avm_gpu_light_core(double *p_right, double *p_front, double *p_left, double *p_rear)
{
    int i;
    int thr = 15;
...
*/
int AvmGpu::avm_gpu_light_balance(unsigned char *p0, unsigned char *p1,
        unsigned char *p2, unsigned char *p3)
{
    ...
    int is_hide = 0;
    avm_gpu_hide_camera_logic(&is_hide);
    if (is_hide)
    {
    ...
    if (m_s_is_share_lb && (0 == m_is_do_lb_process))
    {
        for (i = 0; i < 4; i++)
        {
            m_color_k[i] = AvmGpu::m_s_share_lb.m_color_k[i];
        }
        return 0;
    }
    ...
    if (m_s_is_share_lb)
    {
        for (i = 0; i < 4; i++)
        {
            AvmGpu::m_s_share_lb.m_color_k[i] = m_color_k[i];
        }
    }
    ...
    if (0 == div)
    {
        avm_gpu_light_calc_avg_rgb(0, p0);
    }
    else if (1 == div)
    {
        avm_gpu_light_calc_avg_rgb(1, p1);
    }
    else if (2 == div)
    {
        avm_gpu_light_calc_avg_rgb(2, p1);
    }
    else if (3 == div)
    {
        avm_gpu_light_calc_avg_rgb(3, p2);
    }
    else if (4 == div)
    {
        avm_gpu_light_calc_avg_rgb(4, p2);
    }
    else if (5 == div)
    {
        avm_gpu_light_calc_avg_rgb(5, p3);
    }
    else if (6 == div)
    {
        avm_gpu_light_calc_avg_rgb(6, p3);
    }
    else if (7 == div)
    {
        avm_gpu_light_calc_avg_rgb(7, p0);
    }
    else if (8 == div) // step 8, calc dst color_k
    {
        ...
        double left_val  = 1.0;
        double rear_val  = 1.0;
        if (0 == avm_gpu_light_core(&right_val, &front_val, &left_val, &rear_val))
        {
...
int AvmGpu::avm_gpu_light_balance_process()
{
    ...
    if (0 == m_is_use_bmp)
    {
        ...
        avm_gpu_light_balance(m_texture_vaddr[tex_right], m_texture_vaddr[tex_front], 
            m_texture_vaddr[tex_left], m_texture_vaddr[tex_rear]);
#endif
    }
    else
    {
        avm_gpu_light_balance(m_texture_vaddr[0], m_texture_vaddr[1],
...
int AvmGpu::avm_gpu_load_db_camera(char *p_file, float **p_dst, int *p_point_num)
{
    struct stat statbuff;
    if (stat(p_file, &statbuff) < 0)
    {
        AVM_ERR("AvmGpu::avm_gpu_load_db_camera. stat file faild\n");
        return 1;
    }
    unsigned long filesize = statbuff.st_size;
    ...
    *p_dst = (float *)memalign(128, filesize);
    if (NULL == *p_dst)
    {
        AVM_ERR("AvmGpu::avm_gpu_load_db_camera. alloc mem faild\n");
        return 1;
    }
    fread(*p_dst, filesize, 1, file_r);
    fclose(file_r);

    AVM_LOG("AvmGpu::avm_gpu_load_db_camera. inst_id=%d, file='%s' success\n", 
        m_inst_id, p_file);
...
int AvmGpu::avm_gpu_load_db_fisheye(char *p_file)
{
    struct stat statbuff;
    FILE *file_r = NULL;
    if (stat(p_file, &statbuff) < 0)
    {
        AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. stat file faild\n");
        AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. use init default fisheye value\n");
        //return 1;
    }
    else
    {
    ...
    if (re != 0)
    {
		AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. read fisheye front config faild\n");
		AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. fish front default\n");
		ini_vaild = 0;
		//return 1;
    }
    if (ini_vaild)
    ...
    else
    {
        ...
        AVM_LOG("avm_gpu_load_db_fisheye. f: offset(%f)\n", offset_x);
        ...
        if (re != 0)
        {
            AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. read fish front auto config faild\n");
            AVM_ERR("AvmGpu::avm_gpu_load_db_fisheyes. fish front auto set default 0, height\n");
            lt_y = 0;
    ...
    if (re != 0)
    {
		AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. read rear config faild\n");
		AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. rear set as default\n");
		ini_vaild = 0;
		//return 1;
    }
    if (ini_vaild)
    ...
    else
    {
        ...
        AVM_LOG("avm_gpu_load_db_fisheye. r: offset(%f)\n", offset_x);
        if(camera_offset_x_max*2 < offset_x)
        {
            offset_x = 0;
        }
        ...
        if (re != 0)
        {
            AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. read fish rear auto config faild\n");
            AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. fish fish rear auto set as default\n");
            lt_y = 0;
            rb_y = m_texture_height;
            //return 1;
        }
    ...
    if (re != 0)
    {
		AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. read front clip config faild\n");
        AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. set front clip config as default\n");
        ini_vaild = 0;
		//return 1;
    }
    if (ini_vaild)
    ...
    else
    {
        ...
        float offset_x = m_fisheye_front.cx - m_texture_width/2.0;
        AVM_LOG("avm_gpu_load_db_fisheye. fc: offset(%f)\n", offset_x);
        if(camera_offset_x_max*2 < offset_x)
        {
            offset_x = 0;
        }
        ...
        if (re != 0)
        {
            AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. read front clip auto config faild\n");
            AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. set front clip auto as default\n");
            lt_y = 0;
            rb_y = m_texture_height;
            //return 1;
        }
    ...
    if (re != 0)
    {
		AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. read rear clip config faild\n");
        AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. set rear clip config as default\n");
        ini_vaild = 0;
		//return 1;
    }
    if (ini_vaild)
    ...
    {
        float offset_x = m_fisheye_rear.cx - m_texture_width/2.0;
        AVM_LOG("avm_gpu_load_db_fisheye. rc: offset(%f)\n", offset_x);
        if(camera_offset_x_max*2 < offset_x)
        {
            offset_x = 0;
        }
        ...
        if (re != 0)
        {
            AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. read rear clip auto config faild\n");
            AVM_ERR("AvmGpu::avm_gpu_load_db_fisheye. set rear clip auto as default\n");
            lt_y = 0;
            rb_y = m_texture_height;
            //return 1;
        }
...
void AvmGpu::avm_gpu_hide_camera_logic(int *p_is_hide)
{
    // process hide camera. 
