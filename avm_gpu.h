...
#define AVMGPU_LOG(...)     AVM_LOG(__VA_ARGS__)
#define AVMGPU_ERR(...)     AVM_ERR(__VA_ARGS__)
...
/**
 * @brief The AVM_VIEW_E enum
 * descrip all view port which avm support
 */
typedef enum _AVM_VIEW_ENUM_
{
    AVM_VIEW_AUTO = 0,          /**< bird view */
    AVM_VIEW_3D_FRONT = 1,
    AVM_VIEW_3D_REAR = 2,
    AVM_VIEW_3D_LEFT = 3,
    AVM_VIEW_3D_RIGHT = 4,
    AVM_VIEW_2D_FRONT = 5,
    AVM_VIEW_2D_REAR = 6,
    AVM_VIEW_CAMERA_FRONT = 7,
    AVM_VIEW_CAMERA_REAR = 8,
    AVM_VIEW_CAMERA_LEFTRIGHT = 9,  /**< left camera and right camera at screen left and right */

    AVM_VIEW_3D_LEFT_FRONT = 10,
    AVM_VIEW_3D_LEFT_CENTER = 11,
    AVM_VIEW_3D_RIGHT_FRONT = 12,
    AVM_VIEW_3D_RIGHT_CENTER = 13,

    AVM_VIEW_CAMERA_FISH_FRONT = 14,    /*< origin fish eye view, clip less */
    AVM_VIEW_CAMERA_FISH_REAR = 15,
    AVM_VIEW_CAMERA_LEFTRIGHT_CPY = 16,     /**< same as AVM_VIEW_CAMERA_LEFTRIGHT */
    AVM_VIEW_CAMERA_FISH_CLIP_FRONT = 17,   /**< fisheye clip, fit left view width */
    AVM_VIEW_CAMERA_FISH_CLIP_REAR = 18,

    AVM_VIEW_3D_FREE = 19,      /**< in 3d free 360 degree rotate */
    
    AVM_VIEW_LAST = AVM_VIEW_3D_FREE,
    AVM_VIEW_UNKNOW = 99
}AVM_VIEW_E;
...
*/
typedef struct _avmgpu_share_vbo_struct_
{
    GLuint m_avm_table_vbo;
    GLuint m_avm_idx_vbo;
    GLuint m_avm_single_front_camera_vbo;
    GLuint m_avm_single_rear_camera_vbo;
    GLuint m_avm_two_camera_vbo;
    GLuint m_avm_fisheye_front_vbo;
    GLuint m_avm_fisheye_rear_vbo;
    GLuint m_avm_fisheye_clip_front_vbo;
    GLuint m_avm_fisheye_clip_rear_vbo;
    int m_avm_mem_st_num;
...
*/
typedef struct _avmgpu_share_texure_struct_
{
...
}avmgpu_share_texure_s;


/**
* 3d stitch view, light balance share
*/
typedef struct _avmgpu_share_ltbalance_struct_
{
    float m_color_k[4];    /**< four image's rgb param. */
    float m_fps;
}avmgpu_share_ltbalance_s;
...
public:
    ...
    */
    AvmGpu(int car_type, int proj_width, int proj_height,
    ...
    */
    ...
    float avm_gpu_postion_get_auto_height()
    {
    ...
    * @param proj_h
    */
    void avm_gpu_update_wh(int proj_w, int proj_h)
    {
        m_projWidth = proj_w;
        m_projHeight = proj_h;
    }
    ...
    /**
     * @brief avm_gpu_set_view, set module output view
     *        ouput view will change immediately, if you want to change the view dynamically, you can use func 'avm_gpu_set_view_dyn_trans'
     * @param view_enum, value in AVM_VIEW_E
     * @param is_force, if force==0 and current_view==view_enum, this func will do nothing. otherwise will do setting view process
     */
    int avm_gpu_set_view(int view_enum, int is_force = 0);

    /**
    * @brief dyn trans from cur view to dst view
    *        only if both views are 3D, have dynamic animation, otherwise equal to 'avm_gpu_set_view'
    */
    int avm_gpu_set_view_dyn_trans(int view_enum);

    /**< get current view_enum, AVM_VIEW_E */
    int avm_gpu_get_view()
    {
    ...
    */
    void avm_gpu_set_rotate_center_coord(float center_x, float center_y)
    {
    ...
    */
    int avm_gpu_set_rotate(int is_angle, int x_det, float view_angle);

    /**
    * @brief rotation 3d view, both horizontal and vertical directions can be rotated
    * @param x_det
    */
    int avm_gpu_omni_directional_rotate(int x_det, int y_det);

    /**
    * @brief Horizontal rotation 3d view
    *        the difference between this function and 'avm_gpu_set_rotate' is that this func use det rotation angle
    * @param det_angle, 
    */
    int avm_gpu_set_rotate_det_angle(float det_angle);

    /**
    * @brief get current 3d view's rotate angle(hor rotate)
    */
    float avm_gpu_get_rotate_angle()
    {
    ...
    */
    void avm_gpu_get_v_p(glm::mat4 *p_v, glm::mat4 *p_p);

    int avm_gpu_check_in_3d_view(int view_enum);

    int avm_gpu_check_in_pure_3d_view(int view_enum);

    /**
    * @brief hide stitch camera, can be used when door is open
    * @param 0 not hide, 1 hide. -1 unchange
    */
    void avm_gpu_hide_stitch(int is_hide_right, int is_hide_front,
        int is_hide_left, int is_hide_rear);

    /**
     * @brief avm_gpu_process, process don't change width and height
...
private:
    ...
    int avm_gpu_update_dev_img_addr();
    int avm_gpu_update_pcenv_img_addr(int is_bmp_seq);
    int avm_gpu_update_img_addr();

    /**< update viv texture based on texture image address */
    int avm_gpu_update_viv_gltex();

    /**< update image tex */
    int avm_gpu_update_bmp_gltex();

    /**< generate opengl texture */
    int avm_gpu_process_pre_texture();  

    /**< @brief avm_gpu_texture_init, create texture */
    int avm_gpu_texture_init();

     /**< @brief opengl vbo init */
    int avm_gpu_vbo_init();    

    /**< load llight balance data */
    int avm_gpu_light_balance_init();
    ...
    */
    int avm_gpu_normal_front_process(int type);

    
    int avm_gpu_normal_rear_process(int type);
    int avm_gpu_normal_two_process();
    int avm_gpu_view_dyn_process();
    float avm_gpu_get_view_deg(int view_enum);    
    int avm_gpu_load_db_light(char *p_file);   /**< read db data and set light array data */
    void avm_gpu_light_calc_avg_rgb(int area_id, unsigned char *p_img);
    int avm_gpu_light_core(double *p_right, double *p_front, double *p_left, double *p_rear);
    int avm_gpu_light_balance(unsigned char *p0, unsigned char *p1,
        unsigned char *p2, unsigned char *p3);    /**< light balance process, gen kr, kg, kb param */
    int avm_gpu_light_balance_process();    /**< light balance process function */
	int avm_gpu_load_db_camera(char *p_file, float **p_dst, int *p_point_num);	/**< load single or two camera calib data */
    void avm_gpu_hide_camera_logic(int *p_is_hide);
    int avm_gpu_load_db_fisheye(char *p_file);
...
private:
    ...
    GLint m_loc_texcoord_ex;  /**< location of ex texcoord, see avm_gpu_table.h for detail */
...
private:
    ...
    float m_rotate_max_z_angle; /**< (3d view) max z rotate angle, used in avm_gpu_omni_directional_rotate() func */
...
private:
    char *m_p_db_file_name;     /**< avm table file name string */
    char *m_p_vshader_name;
    char *m_p_fshader_name;
    float *m_p_avm_mem;         /**< table mem. fill with avm_buffer_data_s */
    unsigned int *m_p_avm_idx_mem;  /**< table idx mem*/
    int m_avm_mem_st_num;       /**< mem have how many struct(avm_buffer_data_s) */
...
private:
    ...
    static avmgpu_share_vbo_s m_s_hare_vbo;
    static avmgpu_share_texure_s m_s_share_tex;
    static avmgpu_share_ltbalance_s m_s_share_lb;
};
